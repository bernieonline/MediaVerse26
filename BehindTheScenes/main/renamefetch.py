...
class getChangeWindow(qtw.QWidget):
    def __init__(self, *args, **kwargs):
        ...
        self.test = self.ui.testButton
        self.test.clicked.connect(self.getFolder)
        ...
        self.QAButton.clicked.connect(self.loadQAChecker)
    ...
    def getWDriveRecord(self):
        ...
        while index == 0:
            ...
            print("still getting a id and file are ", ID,File)
    ...
    def getMatchingRecords(self, ID, File, library):
        sizeLibrary = len(library)
        if sizeLibrary > 0:
            moreRecords = True
        else:
            moreRecords = False

        # need to process main section if moreRecords = True
        if moreRecords:
            similarList = []
            index = 0
    ...
    def passinarow(self):
        ...
        it checks whether its visible if not - ts ignored - and if so
        ...
        if each stage gets OK the final step is to process the parent row
        will not update library TRUE flag unless every step is OK
        but will carry out a global rename and set new name in libary
        flag will be set if this faial step completes OK and visible signal set to tell user to process the next file
        :return:
        '''
        ...
        for x in self.parentButtons:
            if x.isChecked():
                parentRow = counter         # row number of parent
                parentfilename = self.namelabel[counter].text()     # file name of parent
            counter = counter + 1
        ...
        nameglobal = self.ui.masterFileNameTxt.text()       # gets global change name without file extension
        if len(nameglobal) < 1:                             # no global change
            globalaction = False
        else:
            globalaction = True                             # sets up global name change
        ...
        #   check if not visible >> skip
        #   check if not checked >> skip
        #   the rest we process one by one sending details tp 'processarow'
        #   ================================================================================================
        counter = 0
        successAllRows = True                           # error checks set this to false
        for x in self.recordIDLabel:                    # loop through id labels in display one by one
            if not x.isHidden():                        # if not hidden row
                if counter != parentRow or globalaction:       # if not parent row
                                                               # or if it is a parent row but global change needed
                    if self.rencheckBox[counter].isChecked():  # if the row is checked for renaming
                        ...
                        file = self.namelabel[counter].text()   # now we have all variables in place and pass to act

                        # result is true or false if rename on disk is completed
                        # also sets nameconf label = true
                        result = self.processarow(idno, path, file, globalaction, nameglobal, parentfilename)
                        # returns 1 or 0 if library update label has content
                        libUpdate = sql.getLibCheck(idno)
                        print("libupdate value 0 or 1", libUpdate)

                        # if rename worked
                        print("result ", result)
                        if result:
                            # set nameconf text to 1+whatever it returned in libupdate 0 or 1
                            self.chlabel[counter].setText("1"+libUpdate)    # if update flag set in lib show 1
                            # if it was previously set to false
                            if libUpdate == "0":
                                successAllRows = False
                        # if rename on disk failed
                        else:
                            print("ggggggggg")
    ...
    def processarow(self, idno, path, file, globalaction, nameglobal, parentfilename):
        ...
        try:
                                                                #   dupecheck = False
                                                                #   print("renaming ", oldName, ",", newName)
                                                                #   print("dupecheck ", 'False')

            myOldFile = os.path.splitext(oldName)
            myNewFile = os.path.splitext(newName)

                                                                #   if myOldFile == myNewFile:
                                                                    #   print("dupechect true ", myOldFile,"--",myNewFile)
                                                                    #   dupeCheck = True
            ...
            os.rename(oldName, newName)
            success = True
            print("now renamed to ", myNewFile)

        except PermissionError:
            # if name change failed on library set flag
            conf = 0
            msf.warningMessagePopUp("Permission Required", "")
            success = False
        ...
        #now update library tag
        try:
            print("ccccccccccccccccccccccccccc")
            # sets nameconf = true for ID if name change worked on disk
            conf = sql.updateLibrary(idno)
        except :
            print("dddddddddddddddddddd")
            # if name change failed on disk set flag
            conf = 0

            # changes parent filename details filename in library
        try:
            conf = sql.updateLibraryFile(idno, parentFileOnly)
        except :
            print("dddddddddddddddddddd")
            # if name change failed on disk set flag
            conf = 0
        ...
        return success
                                                                    ...
                                                                    #   else:
                                                                        #   print("what is dupecheck")
                                                                        #   msf.warningMessagePopUp("Already a file with same name","need to delete one")
                                                                        #   success = False
    ...
    def loadQAChecker(self):
        '''
        reads check boxes and radio buttons
        checks set children
        we want to take a block of files with same name and pass each filename into checkfileqa()
        we get a filename back
        pass this text filename to readQAResult() if size is 0 set library qa flag return result
        keep track of results
        on completion print error list to a text file appending as we go leaving us with a list of duff files
        :return:
        '''
        # get row numbers of files to be checked
        print("qa check 1")
        counter = 0
        checklist = []
        print("qa check 2")
        for check in self.rencheckBox:
            print("qa check 3")
            if check.isChecked():
                print("qa check 4")
                checklist.append(counter)
                # need to get file namepath
                mid = self.recordIDLabel[counter].text()
                path = self.pathlabel[counter].text()
                file = self.namelabel[counter].text()
                fullPath = path+file
                print("qa check 5 ", mid, fullPath)
                print("wwwwwwwwwwwwww")
                self.checkFileQA(str(mid), fullPath)

            counter = counter + 1

        print("These items checked ", checklist)

    def checkFileQA(self, mid, fullPath):
        '''
        reads in a file path
        runs ffmpeg quality check
        creates an output file with the results
        objective is to achieve an empty file
        if empty set qa flag in database 1 else 0 if something is in the file

        :return:
        '''
        print("qa check 6 ", mid)
        # qaCheck is a filename holding the results of a quality check
        qaCheck = ff.checkError(mid, fullPath)
        print("qa check 7")
        print("this file was created ", qaCheck)

    def getFolder(self):
      self.window = FileWindow()
