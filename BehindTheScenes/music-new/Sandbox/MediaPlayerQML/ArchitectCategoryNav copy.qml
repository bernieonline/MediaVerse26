...
Item {
    ...
    Column {
        ...
        Column {
            ...
            TextField {
                id: filterField
                width: parent.width; height: 35; placeholderText: "Filter..."
                color: "white"; leftPadding: 35
                background: Rectangle { color: "#15FFFFFF"; radius: 17 }
            }
            Rectangle {
                    ...
                    Flow {
                        ...
                        Repeater {
                            ...
                            delegate: Button {
                                id: cloudItem
                                padding: 8
                                contentItem: Text { text: modelData; color: cloudItem.hovered ? "black" : "gold"; font.pixelSize: 11 }
                                background: Rectangle { color: cloudItem.hovered ? "gold" : "transparent"; border.color: "gold"; radius: 14 }
                                onClicked: {
                                    var exists = false;
                                    for(var i=0; i < selectedItemsModel.count; i++) { if(selectedItemsModel.get(i).val === modelData) exists = true; }
                                    if(!exists) { selectedItemsModel.append({"val": modelData}); syncMasterRule(); }
                                }
                            }
                        }
                    }
    ...
    // --- FIXED LOGIC BAR (Stays at the bottom) ---
    Row {
        id: footerBar
        ...
        CheckBox {
            ...
            indicator: Rectangle {
                ...
                color: "transparent"
                Rectangle { width: 10; height: 10; x: 4; y: 4; radius: 2; color: "#00F2FF"; visible: filterToggle.checked }
            }
