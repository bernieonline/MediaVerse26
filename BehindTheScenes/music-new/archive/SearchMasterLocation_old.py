...
class Worker2(QThread):
    ...
    def __init__(self, device_name, start_path, myList, myFolders, media_type, master_type):
        ...
        print("worker2 now created ccc:", self.start_path)
    ...
    def run(self):
        ...
        print("running worker2 run method")
        record_counter = 0
    ...
    def complete_traverse(self):
        ...
        print("now traversing 1a:",self.start_path)
        ...
        for dirpath, dirnames, filenames in os.walk(self.start_path):
            ...
            for filename in filenames:
                ...
                if file_ext in extension_to_type:
                    ...
                    print("now traversing 4")
                
                    logging.debug(f"Attempting to create media record for file: {full_path}")
                    try:
                        media_record = self.create_media_record(
                        ...
                        logging.debug(f"Successfully created media record for file: {full_path}")
                    except Exception as e:
                        logging.error(f"Error creating media record for file {full_path}: {e}")
                    
                    # Log the media record
                    logging.info(f"Media record created: {media_record}")

                    
                    try:
                        all_media_records.append(media_record)
                    except Exception as e:
                        logging.error(f"Error appending media record: {e}")
                    
                    #logging.info("entering counter zone")
                     # Increment the counter and print the message
                    #record_counter += 1
                    #logging.info(f"new record AAAA{record_counter}")
                    #if record_counter % 10 == 0:
                        #logging.info(f"new record BBB should emit signal next and counter should be in 10 series {record_counter}")

                        #try:
                            #self.record_counter_signal.emit(record_counter)
                        #except Exception as e:
                            #logging.error(f"Error emitting record_counter_signal: {e}")
        ...
        try:
            ...
            logging.error(f"Error inserting media records: {e}")
...
class SearchMasterLocation(QWidget):
    ...
    def __init__(self, getWin=None, Footer = None, master_progress_bar=None, media_type=None, master_type=None, selected_path=None):
        ...
        print("...........testing class used is master file search.............selected path.............................:",self.selected_path)
    ...
    def run(self):   
        print("running - search master location")
       
        #self.selected_folder = self.selected_path("search folder is:"+self.selected_folder)
    ...
    def traverse_folders(self, start_path):
        ...
        print("creating worker2")
        self.worker2 = Worker2(
        device_name=device_name,
        start_path=self.selected_path,
        myList=self.myList,
        ...
        print("Pulsing bar")
        #self.worker2.record_counter_signal.connect(self.update_filecount_lcd)  # Connect the signal here
        ...
        #this code needs to be here because it cant be called til worker2 has been created
         # Connect the record_counter_signal to update_filecount_lcd
        #if hasattr(self.parent, 'update_filecount_lcd'):
            #self.worker2.record_counter_signal.connect(self.parent.update_filecount_lcd)
          
        #else:
            #print("update_filecount_lcd method not found in parent.")
