...
import srcMedia.GenFuncs 
from srcMedia.GenFuncs import calculate_md5
...
class Worker2(QThread):
    ...
    def __init__(self, getWin,device_name, start_path, myList, myFolders, media_type, master_type):
        super().__init__()
        self.getWin = getWin
        ...
        print(f"Collection date time  is: {self.coll_date}")
    ...
    def complete_traverse(self):
        ...
        for dirpath, dirnames, filenames in os.walk(self.start_path):
            ...
            if 'video_ts' in dirnames_lower or any(filename.lower().endswith('.vob') for filename in filenames):
                ...
                media_record = self.create_media_record(
                    ...
                    collection=f"{self.master_type}:{self.device_name}:{self.start_path}:{self.coll_date}",
                    category="Video",
            ...
            for filename in filenames:
                ...
                if file_ext in extension_to_type:
                    ...
                    if category == 'Video':
                        file_hash = 'None Calculated'
                        self.duplicate = 'X'
                        #too slow to hash large video files
                    else:
        ...
        print(f"End time is: {formatted_end_time}")
...
class SearchMasterLocation(QWidget):
    ...
    def traverse_folders(self, start_path):
        ...
        self.worker2.record_counter_signal.connect(self.getWin.update_filecount_lcd)
